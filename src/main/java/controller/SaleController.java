...
import configuration.GenerateSerialNumeber;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (menu.equalsIgnoreCase("sale")) {
            ...
            switch (action) {
                ...
                case "listSale":
                    //id_Sale_detail, id_Sales, id_Product, quantity,SubTotal
                    
                    List<Sale> sales = saleDao.listAll();
                    request.setAttribute("sale",sales);
                    
                    request.getRequestDispatcher("sale_detail.jsp").forward(request, response);
                    break;
                ...
                case "agregar":
                    ...
                    sale.setDetails(listSaleDetail);
                    item++;

                    for (int i = 0; i < listSaleDetail.size(); i++) {
                        cantTotal = cantTotal + listSaleDetail.get(i).getQuantity();
